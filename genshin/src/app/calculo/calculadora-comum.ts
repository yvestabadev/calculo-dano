...
export class CalculadoraComum implements Calculadora{
    ...
    explicacao(): string {
        ...
        return 'Primeiro se calcula o dano original do talento, multiplicando a porcentagem pelo valor do atributo em questão (ATK, HP, DEF), que resultou em ' +
            ...
            'Soma-se esse resultado com o somador de dano (' + (this.somadorDanoBase * 100).toFixed(2) + '% = ' + danoBaseSomado.toFixed(0) + '). ' + 
            'Sobre este resultado, aplicam-se os seguintes bônus, um em sequência do outro: (1) o bônus de dano do elemento correspondente (' + 
            (this.bonusDanoElemento * 100).toFixed(2) + '% = ' + danoComBonusElemental.toFixed(0) + '); (2) média do dano crítico de acordo com a taxa crítica (' + (this.taxaCritica * 100) +
            '% de ' + (this.danoCritico * 100).toFixed(2) + '% = ' + mediaDanoCritico.toFixed(0) +'%) resultando em ' + resultadoSemDefesa.toFixed(0) +
            '. Aplica-se também a subtração da defesa média de inimigos de nível 90 de acordo com o nível informado do personagem ('+
            this.nivel +') que é aproximadamente ' + CalculadoraDefesa.valorDefPorcentagem(this.nivel) + '. ' +
            'Resultado final: ' + (resultadoSemDefesa * CalculadoraDefesa.calcular(this.nivel)).toFixed(0);
    }
